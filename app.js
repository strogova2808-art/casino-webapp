class CasinoApp {
    constructor() {
        ...
        this.prizesConfig = {
            3: {'bear': {name: 'Мишка', value: 21}, 'rose': {name: 'Роза', value: 32}, 'ring': {name: 'Кольцо', value: 129}, 'rocket': {name: 'Ракета', value: 63}},
            9: {'rose': {name: 'Роза', value: 32}, 'rocket': {name: 'Ракета', value: 63}, 'candy': {name: 'Candy Cane', value: 357}, 'b-day': {name: 'B-Day Candle', value: 378}, 'desk': {name: 'Desk Calendar', value: 315}, 's-box': {name: 'Snake Box', value: 389}},
            15: {'ring': {name: 'Кольцо', value: 129}, 'rocket': {name: 'Ракета', value: 63}, 'candy': {name: 'Candy Cane', value: 357}, 'b-day': {name: 'B-Day Candle', value: 378}, 'desk': {name: 'Desk Calendar', value: 315}, 's-box': {name: 'Snake Box', value: 389}, 'Tama': {name: 'Tama Gadget', value: 525}, 'Hypno': {name: 'Hypno Lollipop', value: 525}, 'Etern': {name: 'Eternal Candle', value: 840}, 'HePo': {name: 'Hex Pot', value: 735}}
        };

        this.weights = {
            3: {'bear': 30, 'rose': 20, 'ring': 10, 'rocket': 20},
            9: {'rose': 25, 'rocket': 12, 'candy': 8, 'b-day': 6, 'desk': 5, 's-box': 4},
            15: {'ring': 15, 'rocket': 12, 'candy': 8, 'b-day': 6, 'desk': 5, 's-box': 4, 'Tama': 3, 'Hypno': 2, 'Etern': 1, 'HePo': 1}
        };

        this.stickerPaths = {
            'bear': './stickers/bear.gif', 'rose': './stickers/rose.gif', 'ring': './stickers/ring.gif', 'rocket': './stickers/rocket.gif',
            'candy': './stickers/candy.gif', 'b-day': './stickers/b-day.gif', 'desk': './stickers/desk.gif', 's-box': './stickers/s-box.gif',
            'Tama': './stickers/Tama.gif', 'Hypno': './stickers/Hypno.gif', 'Etern': './stickers/Etern.gif', 'HePo': './stickers/HePo.gif'
        };
    ...
    showProxyBanner() {
        ...
        banner.style.cssText = `position:fixed;top:0;left:0;right:0;background:linear-gradient(135deg,#ff6b6b,#ee5a52);color:white;padding:12px 15px;text-align:center;font-weight:600;font-size:14px;z-index:10000;display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;`;
        banner.innerHTML = `<span>🔧 Консоль-бот | Запросы отправляются на модерацию</span>
            <button onclick="casino.goToMainBot()" style="background:white;color:#ff6b6b;border:none;padding:6px 12px;border-radius:8px;font-weight:bold;cursor:pointer;font-size:12px;">🎰 Основной бот</button>`;
    ...
    updateUserAvatar(user) {
        ...
        if (user.photo_url) {
            ...
            const colors = [['#7f2b8f','#c44569'], ['#2b8f8c','#69c4a4'], ['#8f2b2b','#c46945'], ['#2b8f4a','#45c469']];
    ...
    saveUserProfile(user) {
        try {
            ...
            const profileData = {user_id:user.id, username:user.username, first_name:user.first_name, last_name:user.last_name||'', photo_url:user.photo_url||'', language_code:user.language_code||'ru', last_seen:new Date().toISOString()};
    ...
    setupFallbackData() {
        ...
        this.userData = {id: randomId, first_name: 'Игрок', username: 'demo_user'};
    ...
    async sendToNetlify(data) {
        try {
            const response = await fetch(this.netlifyUrl, {
                ...
                body: JSON.stringify({...data, user_id: this.userId, bot_type: this.currentBot, timestamp: Date.now(), username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'})
            });
            ...
            return {success: true, message: 'Данные сохранены локально', user_data: {user_id: this.userId, balance: this.userBalance, games_played: this.gamesPlayed, total_won: this.totalWon, biggest_win: this.biggestWin, wins_count: this.winsCount}};
        }
    }

    async sendToBot(data) {
        const result = await this.sendToNetlify(data);
        if (result.success && result.user_data) this.updateFromServerData(result.user_data);
    ...
    updateFromServerData(serverData) {
        if (serverData.balance !== undefined) {
            ...
            if (serverData.balance !== 222) this.isNewUser = false;
        }
    ...
    async loadInitialData() {
        ...
        if (this.isNewUser) this.userBalance = 222;
        
        if (window.Telegram && Telegram.WebApp) {
            try {
                const data = {action: 'get_initial_data', user_id: this.userId, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf', is_new_user: this.isNewUser};
                const result = await this.sendToNetlify(data);
                if (result.success && result.user_data) this.updateFromServerData(result.user_data);
            } catch (error) {
    ...
    async saveUserDataToDatabase() {
        ...
        if (window.Telegram && Telegram.WebApp) {
            try {
                const data = {action: 'update_balance', user_id: this.userId, balance: this.userBalance, games_played: this.gamesPlayed, total_won: this.totalWon, biggest_win: this.biggestWin, wins_count: this.winsCount, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'};
    ...
    processDeposit() {
        if (this.selectedDepositAmount > 0) {
            const data = {action: 'deposit_request', user_id: this.userId, amount: this.selectedDepositAmount, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'};
            this.sendToBot(data);
    ...
    withdrawPrize() {
        if (this.currentPrize) {
            const data = {action: 'withdraw_prize', user_id: this.userId, prize: this.currentPrize.name, value: this.currentPrize.value, sticker: this.currentPrize.sticker, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'};
            this.sendToBot(data);
    ...
    loadUserDataFromLocalStorage() {
        try {
            ...
            if (savedData) {
                ...
                this.isNewUser = data.balance === 222;
            }
        } catch (error) {
    ...
    saveUserDataToLocalStorage() {
        try {
            const userKey = `casino_user_${this.userId}`;
            const data = {balance: this.userBalance, gamesPlayed: this.gamesPlayed, totalWon: this.totalWon, biggestWin: this.biggestWin, winsCount: this.winsCount, gameHistory: this.gameHistory, userId: this.userId, username: this.userData?.username, first_name: this.userData?.first_name, last_updated: Date.now(), domain: 'teal-lollipop-dfedaf'};
    ...
    updateBalance() {
        ...
        if (balanceElement) balanceElement.textContent = this.userBalance + ' ⭐';
    }
    ...
    async spinSlot() {
        ...
        try {
            ...
            if (prize) {
                ...
                const gameData = {action: 'game_result', user_id: this.userId, bet_amount: this.currentBet, win: true, prize_name: prize.name, prize_value: prize.value, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'};
                ...
                const gameData = {action: 'game_result', user_id: this.userId, bet_amount: this.currentBet, win: false, bot_type: this.currentBot, username: this.userData?.username, first_name: this.userData?.first_name, domain: 'teal-lollipop-dfedaf'};
                this.sendToBot(gameData);
            }
    ...
    async animateReelSpinning(reelNumber, spinDuration) {
        ...
        for (let i = 0; i < changes; i++) {
            const reelElement = document.getElementById(`reel${reelNumber}`);
            if (!reelElement || !reelElement.classList.contains('spinning')) break;
    ...
    checkWin(spinResult) {
        ...
        if (spinResult.every(sticker => sticker === firstSticker)) {
            ...
            if (prizeConfig) return {name: prizeConfig.name, value: prizeConfig.value, sticker: firstSticker};
        }
        return null;
    }
    ...
    showSection(section) {
        ...
        if (sectionIndex !== -1 && navItems[sectionIndex]) navItems[sectionIndex].classList.add('active');
    }
    ...
    showDepositModal() {
        ...
        document.querySelectorAll('.deposit-option').forEach(option => option.classList.remove('selected'));
...
function spinSlot() { if (casino) casino.spinSlot(); }
function selectBet(bet) { if (casino) casino.selectBet(parseInt(bet)); }
function showSection(section) { if (casino) casino.showSection(section); }
function showDepositModal() { if (casino) casino.showDepositModal(); }
function closeDepositModal() { if (casino) casino.closeDepositModal(); }
function selectDeposit(amount) { if (casino) casino.selectDeposit(parseInt(amount)); }
function processDeposit() { if (casino) casino.processDeposit(); }
function closePrizeModal() { if (casino) casino.closePrizeModal(); }
function withdrawPrize() { if (casino) casino.withdrawPrize(); }
function sellPrize() { if (casino) casino.sellPrize(); }
function toggleQuickSpin() { if (casino) casino.toggleQuickSpin(); }
function toggleMenu() { if (casino) casino.toggleMenu(); }
...
window.addEventListener('orientationchange', function() { setTimeout(() => window.scrollTo(0, 0), 100); });
